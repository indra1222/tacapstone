...
class AutoLayoutService:
    ...
    # Furniture database dengan dimensi real (dalam cm, dikonversi ke meter untuk layout)
    FURNITURE_CATALOG = {
        "SOFA 3 Seat": {"panjang": 2.6, "lebar": 1.0, "quantity": 4, "zone": "living", "priority": 1},
        ...
        "Meja Lingkaran Besar": {"panjang": 0.98, "lebar": 0.98, "quantity": 3, "zone": "living", "priority": 4},
        "Meja Lingkaran Kecil": {"panjang": 0.5, "lebar": 0.5, "quantity": 1, "zone": "living", "priority": 5},
        "Kursi Kayu": {"panjang": 0.98, "lebar": 1.0, "quantity": 3, "zone": "living", "priority": 6},
        "Pas Bunga Small": {"panjang": 0.36, "lebar": 0.36, "quantity": 1, "zone": "decoration", "priority": 10},
        "Pas Bunga Medium": {"panjang": 0.43, "lebar": 0.43, "quantity": 1, "zone": "decoration", "priority": 10},
        "Pas Bunga Large": {"panjang": 0.6, "lebar": 0.6, "quantity": 1, "zone": "decoration", "priority": 10},
        "Stand Lukisan": {"panjang": 0.82, "lebar": 0.72, "quantity": 3, "zone": "decoration", "priority": 9},
        "Lukisan Kecil": {"panjang": 0.6, "lebar": 0.8, "quantity": 3, "zone": "decoration", "priority": 11},
        "Lukisan Besar": {"panjang": 4.25, "lebar": 1.8, "quantity": 1, "zone": "decoration", "priority": 8},
        "Meja Makan": {"panjang": 2.4, "lebar": 1.0, "quantity": 4, "zone": "dining", "priority": 1},
        "Kursi Makan": {"panjang": 0.46, "lebar": 0.75, "quantity": 24, "zone": "dining", "priority": 2},
        "Kursi Pantai": {"panjang": 0.8, "lebar": 2.0, "quantity": 6, "zone": "outdoor", "priority": 7}
    }
    ...
    # Minimum spacing between furniture (dalam meter) - INCREASED untuk HINDARI TUMPANG TINDIH
    MIN_SPACING = 0.8  # 80cm spacing for walking path (AMAN, NO OVERLAP!)
    WALL_MARGIN = 0.5  # 50cm from walls (lebih aman)
    OBSTACLE_MARGIN = 0.7  # 70cm from obstacles (stairs, columns) (lebih aman)
    ...
    def check_collision(x1: float, y1: float, w1: float, h1: float,
        ...
        STRICT COLLISION CHECK - Prevent ANY overlap!
        """
        # Check actual AABB collision (overlap)
    ...
    def calculate_grid_positions(zone_name: str, grid_size: float = 0.3) -> List[Tuple[float, float]]:
        ...
        SMALLER GRID = MORE POSITIONS = BETTER PLACEMENT OPTIONS
        """
        zone = AutoLayoutService.ZONES.get(zone_name)
        if not zone:
            return []
        ...
        positions = []
        x = zone["x_min"] + AutoLayoutService.WALL_MARGIN
        while x < zone["x_max"] - AutoLayoutService.WALL_MARGIN:
            y = zone["y_min"] + AutoLayoutService.WALL_MARGIN
            while y < zone["y_max"] - AutoLayoutService.WALL_MARGIN:
    ...
    def place_furniture_optimized(furniture_name: str, furniture_data: Dict,
        ...
        # Get potential positions in priority order - FINE GRID untuk akurasi
        if zone == "wall":
            ...
            positions = AutoLayoutService.calculate_grid_positions(zone, grid_size=0.25)  # FINER GRID
            # Prioritize center and sides
            center_x = (AutoLayoutService.ZONES[zone]["x_min"] + 
                       AutoLayoutService.ZONES[zone]["x_max"]) / 2
            positions.sort(key=lambda p: abs(p[0] - center_x))
        elif zone == "dining":
            # Dining room: grid layout for tables and chairs
            positions = AutoLayoutService.calculate_grid_positions(zone, grid_size=0.25)  # FINER GRID
        elif zone == "outdoor":
            # Outdoor: spread out evenly
            positions = AutoLayoutService.calculate_grid_positions(zone, grid_size=0.4)  # Still generous
        else:
            # Decoration: fill empty spaces
            positions = AutoLayoutService.calculate_grid_positions(zone, grid_size=0.2)  # VERY FINE
    ...
    def auto_place_all_furniture(room_width: float = 17.0, 
        ...
        Returns optimized layout with high accuracy
        """
        placed_items = []
        failed_items = []
